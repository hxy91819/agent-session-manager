...
- 先根据 cwd/项目路径建立候选“项目/事项”清单，再按交付目标整理。cwd 只是分组线索：服务同一结果的技术步骤合并；交付物、当前状态或下一步不同的独立事项应拆开，并使用具体事项名，不能因路径相同硬塞进一条。
...
- 每个顶层条目都必须带一个或多个项目来源标签，包括“工作概览”“后续跟进”“风险与阻塞”。格式为“N. [高投入] [项目] 事项：进展；下一步：计划”或“- [项目] 内容”。
...
- 每个概览事项必须以且只能以 [高投入]、[中投入]、[低投入] 之一开头，投入标签之后紧跟项目来源标签。
...
- 每个概览事项只占一行，格式仍为“项目/事项：进展；下一步：计划”。进展部分可以使用两到三个短句；当不同事实被挤进一个抽象长句时，必须改成短句。单项目标约 140 字且绝不能超过 180 个字符，超出时拆分独立事项或只保留晨会需要的结果。
...
- 输出结构固定为：
  ...
  ## 后续跟进
  ## 风险与阻塞
- 只有“工作概览”使用有序列表，每个项目或事项一项；每项必须显式包含“下一步：”，已完成且无需继续的低投入事项写“下一步：暂无”。三个章节的每个顶层条目都必须带来源标签。
- “今天”指统计窗口内的 24 小时事件，不要因为日会措辞改变统计周期。
- “后续跟进”、“风险与阻塞”使用无序列表。
- 输出前静默自审：每个顶层条目是否带有全部且仅有的来源项目标签、合并多项目时是否保留多个标签、每个概览事项是否有且只有一个合法投入等级、是否误写百分比、同一结果的技术步骤是否已合并、独立事项是否因共用路径而被硬塞进一条、会议工作是否得到体现、读者能否直接复述每项的对象/结果/剩余工作、业务对象是否被泛化、验证是否写明通过/失败/未完成/待确认、是否仍有妨碍理解的底层细节、完成/计划/讨论是否被混淆。不要输出自审过程，只输出最终${report_kind}。
  ...
  for ((attempt = 1; attempt <= generator_attempts; attempt++)); do
    ...
    if REPORT_MODEL="$model" \
      ...
      if [[ ! -s "$attempt_report" ]]; then
        ...
        printf '\n%s\n' \
          "修正要求：上一版格式校验失败。三个章节的每个顶层条目都必须带一个或多个来源项目标签；工作概览格式为“N. [投入] [项目] 事项”，后续与风险格式为“- [项目] 内容”。合并多项目时保留每个来源标签。工作概览不得出现任何百分比，不得超过 180 个字符，并且必须包含“；下一步：<计划>”或“。下一步：<计划>”；已完成事项写“下一步：暂无”。" \
          >> "$prompt_out"
      fi
    else
